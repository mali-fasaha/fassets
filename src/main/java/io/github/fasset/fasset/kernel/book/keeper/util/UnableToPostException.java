...
/**
 * This exception is thrown when the {@code Transaction} client tries to post a transaction when the sum of the {@code AccountingEntry} items does not evaluate to zero, or when the caller of {@code
 * Transaction#post()} does not first ensure that {@code Cash} amounts of {@code CREDIT} {@code AccountingEntry} items are not equivalent to the {@code Cash} amounts of {@code CREDIT} {@code
