 ...
 * javax.persistence.AttributeConverter} interface. It is applied automatically to any {@link java.time.YearMonth} variable that is being persisted to the database, just before the persistence engines
 * flushes it to the database. Again just before the model is rehydrated with database values, the data is converted back from {@link java.util.Date} to {@link java.time.YearMonth} object maintaining
 * user accessibility in both the database and the application.
 *
 * @author edwin.njeru
 * @version $Id: $Id
 */
